...
bool DSC::PhysicsLogic::Init(	std::shared_ptr<flecs::world> _game, 
								std::weak_ptr<const GameConfig> _gameConfig)
{
	...
	float worldSizeMinY = (*readCfg).at("World").at("ysizemin").as<float>();
	float worldSizeMaxY = (*readCfg).at("World").at("ysizemax").as<float>();
	...
	game->system<const Position>("Cleanup System")
		.each([worldSizeMinX, worldSizeMaxX, worldSizeMinY, worldSizeMaxY](flecs::entity e, const Position& p) {
		if (p.value.x > worldSizeMaxX + 1.5f || p.value.x < worldSizeMinX + -1.5f ||
			p.value.y > worldSizeMaxY + 1.5f || p.value.y < worldSizeMinY + -1.5f) {
				e.destruct();
		}
	});
	// **** COLLISIONS ****
	...
	queryCache = game->query<Collidable, Position, Orientation, Collider>();
	...
	game->system<CollisionSystem>()
		...
		queryCache.each([this](flecs::entity e, Collidable& c, Position& p, Orientation& o, Collider& d) {
			...
			for (int i = 0; i < polysize; ++i) {
				//if (e.has<PlayerBullet>() || e.has<EnemyBomber>()) {
				//	v = { e.get<Collider>()->poly[i].x, e.get<Collider>()->poly[i].y, 1 };
				//	//GW::MATH2D::GVECTOR3F v = { poly[i].x, poly[i].y, 1 };
				//}
				//else {
				//	v = { poly[i].x, poly[i].y, 1 };
				//	//GW::MATH2D::GVECTOR3F v = { poly[i].x, poly[i].y, 1 };
				//}
				// use collider poly now instead of static poly
				GW::MATH2D::GVECTOR3F v = { d.poly[i].x, d.poly[i].y, 1 };
...
bool DSC::PhysicsLogic::Activate(bool runSystem)
{
	if (runSystem) {
...
bool DSC::PhysicsLogic::Shutdown()
{
